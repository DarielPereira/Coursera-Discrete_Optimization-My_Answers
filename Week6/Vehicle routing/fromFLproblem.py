...
def draw_problem(df_facilities, df_customers):
    ...
    # plt.xlim(0, 160000)
    # plt.ylim(0, 160000)
    plt.xlim(-2000, 201200)
    plt.ylim(-2000, 201200)
...
if facility_count+customer_count==75 or facility_count+customer_count==200 or facility_count+customer_count==250:
    ...
    distance_matrix_dict = {(m,n):distance_matrix[int(m),int(n)] for m in facility_indexes for n in customer_indexes}
    ...
    FL_model = create_FL_model(facility_indexes, customer_indexes, distance_matrix_dict, fixed_costs, demand_customers, capacity_facilities, cover_cuts)
    ...
    for update in range(number_updates):
        ...
        close_used_facilities = (df_facilities.loc[(df_facilities['in_Use'] == 1) & (df_facilities['cluster_label'] == df_facilities['cluster_label'].loc[random_facility_index])].index).to_list()
        close_unused_facilities = (df_facilities.loc[(df_facilities['in_Use'] == 0) & (df_facilities['cluster_label'] == df_facilities['cluster_label'].loc[random_facility_index])].index).to_list()

        facility_update_indexes = random.sample(close_used_facilities, min(number_facility_updates,len(close_used_facilities)))
        facility_update_indexes += random.sample(close_unused_facilities, min(number_facility_updates,len(close_unused_facilities)))
...
elif facility_count + customer_count == 3500 or facility_count + customer_count == 2500 or facility_count + customer_count == 4000:
    # problem 6
    ...
    if facility_count + customer_count == 2500:
        ...
        plt.scatter(list(df_facilities.locationx), list(df_facilities.locationy), c=list(df_facilities['cluster_label']),
                    marker='p', s=54)
    ...
    ################################################
    kmeans_facilities = KMeans(n_clusters=(n_clusters / 2), init='k-means++')
    kmeans_facilities.fit(df_facilities[df_facilities.columns[3:5]])
