...
trivial_vehicle_tours = trivial_solution(customers, 4*vehicle_count, vehicle_capacity/2, customer_count)
...
edgeMatrix = np.zeros((vehicle_count*4, customer_count, customer_count))
visitingMatrix = np.zeros((vehicle_count*4, customer_count))
...
random.seed(0)
for iter in range(4*vehicle_count):
    print('Iter:{}'.format(iter))

    temp_selected_vehicles = random.sample(range(4*vehicle_count), 2)
